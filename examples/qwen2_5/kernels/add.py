...
def triton_add(a, b, backend="gpu", transform_script=None, f32=False):
    """
    ...
    Args:
        ...
        f32: NPU only. Keep operands and accumulation in float32 instead of
            casting to bf16. Used for the residual stream, where bf16-native
            accumulation compounds across layers (the transform_script must be
            the f32 variant so the device add stays in f32).
    ...
    if backend == "gpu":
        ...
        npu_dtype = torch.float32 if f32 else torch.bfloat16
        a_npu = a_flat.to(npu_dtype)
        b_npu = b_flat.to(npu_dtype)
        ...
        output = torch.empty(n_padded, dtype=npu_dtype)
