...
EXAMPLES = [
    ...
    },
    {
        ...
        "datatypes": "bf16",
        "description": (
            "End-to-end GPT-2 inference (all four sizes: small/medium/large/xl) "
...
def generate_models_table(base_url=""):
    """Generate markdown table rows for the end-to-end models section."""
    rows = []
    for ex in EXAMPLES:
...
def generate_readme(base_url=""):
    ...
    table_body = "\n".join(table_rows)
    model_rows = generate_models_table(base_url=base_url)
    model_body = "\n".join(model_rows)
...
Complete models\u2014not just single operators\u2014built by composing Triton \
kernels and running across the iGPU and NPU.
